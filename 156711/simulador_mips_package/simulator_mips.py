...
class MIPSSimulator:
    ...
    def set_input_values_safe(self, values: List[int], extra_zeros: int = 10):
        """
        Configura valores de input de forma segura para evitar loops

        Args:
            values: Lista de valores de input
            extra_zeros: Número de zeros extras para evitar esgotamento
        """
        if values:
            # Adiciona valores extras para evitar esgotamento
            safe_values = values.copy() + [0] * extra_zeros
            self.input_values = safe_values
            if self.debug:
                print(
                    f"Input values configurados: {values} + {extra_zeros} zeros extras"
                )
        else:
            self.input_values = [0] * extra_zeros
            if self.debug:
                print(f"Input values padrão: {extra_zeros} zeros")
        ...
        try:
            if mnemonic == "ADD":
                ...
                if hasattr(self, "input_values") and self.input_values:
                    ...
                    # Se input_values existe mas está vazio, usar 0 para evitar loop
                    value = 0
                    self.registers[decoded["rd"]] = value & 0xFFFFFFFF
                    if self.debug:
                        print(f"INPUT R{decoded['rd']} = {value} (auto-zero)")
                else:
                    # Última opção: solicitar input do terminal
                    try:
                        value = input(f"INPUT para R{decoded['rd']}: ")
                        self.registers[decoded["rd"]] = int(value) & 0xFFFFFFFF
                    except (ValueError, EOFError, KeyboardInterrupt):
                        print("Valor inválido ou input cancelado, usando 0")
    ...
    def run(
        ...
        # Configurar inputs de forma segura
        if input_values:
