...
class MemoryTraceMIPSSimulator(MIPSSimulator):
    ...
    def execute_instruction(self, instruction):
        ...
        if opcode == "SW":
            ...
            self.trace_memory_access("SW_BEFORE", address, self.memory[address], f"About to write {value}")
            
            # Executar a operação original
            super().execute_instruction(instruction)
            
            # Trace após a escrita  
            self.trace_memory_access("SW_AFTER", address, self.memory[address], f"Written {value}")
            
        elif opcode == "LW":
            # Interceptar LW para trace
            rs = self.get_register_number(args[1].split("(")[1].rstrip(")")) if "(" in args[1] else self.get_register_number(args[1])
            ...
            self.trace_memory_access("LW_BEFORE", address, old_value, f"About to read to R{rt}")
            
            # Executar a operação original
            super().execute_instruction(instruction)
            
            # Trace após a leitura
            self.trace_memory_access("LW_AFTER", address, old_value, f"Read {old_value} to R{rt}")
    ...
    def print_memory_trace(self):
        ...
        for entry in self.memory_trace:
            print(f"CICLO {entry['cycle']:3d} | PC={entry['pc']:2d} | {entry['operation']:<10} | "
