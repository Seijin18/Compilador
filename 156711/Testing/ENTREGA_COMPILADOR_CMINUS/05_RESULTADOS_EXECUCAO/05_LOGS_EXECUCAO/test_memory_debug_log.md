# Relatório de Compilação - test_memory_debug.c
## Data: 2025-09-23 12:21:38
## Status: ✅ SUCESSO

### 📄 Arquivo Original
Localização: CMinusCodes\tests\test_memory_debug.c
Tamanho: 540 bytes
...
ADD_INSTRUCTION[32]: LI (op=27) rs=R0 rt=R17 rd=R0 imm=0
...
ADD_INSTRUCTION[38]: LI (op=27) rs=R0 rt=R21 rd=R0 imm=1
...
ADD_INSTRUCTION[43]: OUTPUTREG (op=32) rs=R24 rt=R0 rd=R0 imm=0
DEBUG: get_register_for_variable('t12', 'main')
DEBUG: Registrador livre encontrado: R25
DEBUG: Alocando 't12' -> R25
ADD_INSTRUCTION[44]: LI (op=27) rs=R0 rt=R25 rd=R0 imm=2
...
ADD_INSTRUCTION[45]: SLL (op=6) rs=R25 rt=R0 rd=R26 imm=2
Global array: GP=28, offset=0
Generating: ADDI R27, GP, 0 (base address)
ADD_INSTRUCTION[46]: ADDI (op=15) rs=R28 rt=R27 rd=R0 imm=0
  ADDI PREVIEW: Will output 'ADDI R27, R28, 0'
Generating: ADD R27, R27, R26 (final address)
ADD_INSTRUCTION[47]: ADD (op=0) rs=R27 rt=R26 rd=R27 imm=0
...
ADD_INSTRUCTION[48]: LW (op=25) rs=R27 rt=R2 rd=R0 imm=0
...
ADD_INSTRUCTION[49]: OUTPUTREG (op=32) rs=R2 rt=R0 rd=R0 imm=0
DEBUG: get_register_for_variable('t14', 'main')
DEBUG: Liberando R3 (era temp_reg_0)
DEBUG: Alocando 't14' -> R3
ADD_INSTRUCTION[50]: LI (op=27) rs=R0 rt=R3 rd=R0 imm=3

>>> PROCESSING QUAD: LOAD data[t14] -> t15

=== DEBUG ARRAY ACCESS ===
Array: 'data', Index: 't14', Result: 't15', LOAD
Symbol found: name='data', offset=0, global=YES, is_array=YES, size=4
...
Index register: R3 (loaded from 't14')
DEBUG: get_register_for_variable('temp_reg_7', 'main')
DEBUG: Liberando R4 (era addr_reg_0)
DEBUG: Alocando 'temp_reg_7' -> R4
DEBUG: get_register_for_variable('addr_reg_7', 'main')
DEBUG: Liberando R1 (era t0)
DEBUG: Alocando 'addr_reg_7' -> R1
Temp register: R4 (temp_reg_7), Address register: R1 (addr_reg_7)
Generating: SLL R4, R3, 2 (index * 4)
ADD_INSTRUCTION[51]: SLL (op=6) rs=R3 rt=R0 rd=R4 imm=2
Global array: GP=28, offset=0
Generating: ADDI R1, GP, 0 (base address)
ADD_INSTRUCTION[52]: ADDI (op=15) rs=R28 rt=R1 rd=R0 imm=0
  ADDI PREVIEW: Will output 'ADDI R1, R28, 0'
Generating: ADD R1, R1, R4 (final address)
ADD_INSTRUCTION[53]: ADD (op=0) rs=R1 rt=R4 rd=R1 imm=0
  ADD PREVIEW: Will output 'ADD R1, R1, R4'
DEBUG: get_register_for_variable('t15', 'main')
DEBUG: Liberando R6 (era t3)
DEBUG: Alocando 't15' -> R6
LOAD: Dest register R6 (to 't15')
Generating: LW R6, 0(R1) (load value from address)
ADD_INSTRUCTION[54]: LW (op=25) rs=R1 rt=R6 rd=R0 imm=0
  MEM PREVIEW: Will output 'LW R6, 0(R1)'
...
ADD_INSTRUCTION[55]: OUTPUTREG (op=32) rs=R6 rt=R0 rd=R0 imm=0
ADD_INSTRUCTION[56]: HALT (op=30) rs=R0 rt=R0 rd=R0 imm=0
